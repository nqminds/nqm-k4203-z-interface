module.exports = (function() { 
  ...
  var log = require("debug")("nqm-k4203-z-interface:dongle");
  ...
  var rebootDevice = function() {
    //Vendor API call to reboot the device.
    postData({ goformId: "REBOOT_DEVICE" });
  };
  
  var getParam = function(paramName, cb) {
    getData({ cmd: paramName }, function(err, response, body) {
      if (err || !body) {
        cb(err || new Error("invalid response - no body"));
      } else {
        cb(err, body);
  ...
  var connect = function(cb) {
    postData({ goformId : "CONNECT_NETWORK" }, cb);
  };

  var disconnect = function(cb) {
    postData({ goformId : "DISCONNECT_NETWORK" }, cb);
  };

  var autoConnect = function(cb) {
    postData({ goformId: "SET_CONNECTION_MODE", ConnectionMode: "auto_dial" }, cb);
  };

  var resetRemindFlag = function(cb) {
    postData({ goformId: "SET_REMIND_FLAG", sms_remind: 0, redirect_flag: "" }, cb);
  };
  ...
  return {
    ...
    resetRemindFlag: resetRemindFlag
  };
}());
